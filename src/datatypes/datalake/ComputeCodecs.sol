...
library ComputationalTaskCodecs {
    ...
    /// @dev Encodes a ComputationalTask.
    /// @param task The ComputationalTask to encode.
    function encode_task(ComputationalTask memory task, bytes memory encodedDatalake)
        internal
        pure
        returns (bytes memory)
    {
        return abi.encode(
            TaskCode.Module, encodedDatalake, abi.encode(task.aggregateFnId, task.operatorId, task.valueToCompare)
        );
    }
